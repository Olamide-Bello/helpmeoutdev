...
const Footer:React.FC = () => {
  return (
    <div className="md:h-[347px] py-[98px] w-full bg-primary-600">
      <div className="container px-3 xs:px-5 ss:px-16 md:px-[100px] gap-8 md:gap-0 flex flex-col md:flex-row justify-between items-start">
            ...
            return (
              <div key={item.header} className="text-white text-[16px] ">
                ...
                <div className="font-Work-Sans flex flex-col gap-[12px] md:gap-[24px]">
                  <Link href={item.links.link1.href}>{item.links.link1.title}</Link>
                  <Link href={item.links.link2.href}>{item.links.link2.title}</Link>
                  <Link href={item.links.link3.href}>{item.links.link3.title}</Link>
